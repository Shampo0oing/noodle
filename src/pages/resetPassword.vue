<script src="../../../noodle-server/routes/resetPassword.js"></script>
<template>
        ...
        <div class="main2">
          <div class="texts">
            ...
            <span class="under-text"
              >Pas de probleme, nous vous enverrons un email!</span
            >
          </div>
          <div class="main-inputs">
            <div class="input-box">
              <div class="box">
                ...
                <span v-if="state.wrong" class="error">
                  cet email n'existe pas</span
                >
                <span v-if="state.success" class="success">
                  un email a été envoyer</span
                >
              </div>
              <v-btn
                v-on:click="sendForm"
              ...
              <v-btn v-on:click="goBack" v-ripple="false" text class="btn"
                >retour</v-btn
              >
            </div>
          </div>
...
export default {
      ...
      return {
        email: {
          required: helpers.withMessage(
            "Veuiller ecrire votre email",
            required
          ),
          email: helpers.withMessage("Veuiller écrire un email valide", email),
        },
      };
    });
  ...
  methods: {
    sendForm() {
      this.v$.email.$touch();
      if (!this.v$.$error) {
        fetch("http://localhost:8080/reset-pass/reset", {
          method: "POST",
          credentials: "include",
          headers: {
